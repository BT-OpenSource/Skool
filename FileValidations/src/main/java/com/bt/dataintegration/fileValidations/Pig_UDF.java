...
public class Pig_UDF extends EvalFunc<DataBag>{
		...
		try{
            ...
            if (value != null && value.length() > 0) {
            	...
            	for(int i =1;i<x.length;i++)
            		temp = temp+x[i];
				fieldArr = temp.split(delimiter);
				if(!("".equals(this.fileTrailerKeyword))){
					
					/*if ("FILEHEADER".equalsIgnoreCase(fieldArr[0]) || 
							fieldArr[0].startsWith("@") ||
							"FILETRAILER".equalsIgnoreCase(fieldArr[0])) {*/
					if(this.fileTrailerKeyword.equalsIgnoreCase(fieldArr[0])){
						String[] firstValues = { Constants.INVALID_RECORD};
